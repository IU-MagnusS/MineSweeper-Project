...
public class GamePanel extends JPanel implements MouseListener {
	...
	private int h;
	private int boom;
	private Stack<ButtonPlay[][]> previousStates = new Stack<>();
	...
	public void mousePressed(MouseEvent e) {
		...
		// Push the current state of the board onto the stack
        ButtonPlay[][] currentBoard = p2.getArrayButton();
        ButtonPlay[][] boardCopy = new ButtonPlay[currentBoard.length][currentBoard[0].length];
        for (int i = 0; i < currentBoard.length; i++) {
            for (int j = 0; j < currentBoard[i].length; j++) {
                boardCopy[i][j] = new ButtonPlay(currentBoard[i][j]);
            }
         
        }
        previousStates.push(boardCopy);
    }
	...
	public void undo() {
        if (!previousStates.isEmpty()) {
            ...
            ButtonPlay[][] previousBoard = previousStates.pop();

            // Create a deep copy of the board
            ButtonPlay[][] newBoard = new ButtonPlay[previousBoard.length][previousBoard[0].length];
            for (int row = 0; row < previousBoard.length; row++) {
                for (int col = 0; col < previousBoard[row].length; col++) {
                    newBoard[row][col] = new ButtonPlay(previousBoard[row][col]);
                }
            }
	        
	        // Set the game board to the previous state
	        p2.setArrayButton(newBoard);
	        p2.revalidate();
	        p2.repaint();
	    }
	   else {
